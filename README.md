...
local function refreshSpeedModeLabel()
    if mobBtnRefs.carrySpeed then mobBtnRefs.carrySpeed(speedMode) end
...
local function toggleCarryMode()
    if laggerToggled or laggerCarryToggled then
        laggerToggled = false; laggerCarryToggled = false; laggerPhase = 0; speedMode = true
...
local function updateAutoSwitchSpeed()
    ...
    _autoSwitchWasSteal = isStealSpeed; speedMode = isStealSpeed
...
function saveConfig()
    ...
    local cfg = {
        ...
        carryMode = speedMode, laggerMode = laggerToggled, laggerCarryMode = laggerCarryToggled,
        ...
        batV2Enabled = batV2Enabled, autoSwitchSpeed = autoSwitchSpeedEnabled,
        skyTheme = currentSkyTheme, uiScale = uiScaleValue,
        stealBarPos = stealPos,
    }
    pcall(function() writefile("PretoDuelsConfig.json", HS:JSONEncode(cfg)) end)
end

task.spawn(function() while task.wait(5) do saveConfig() end end)

-- ============================================================
-- HEAD LABELS (removed Discord label)
-- ============================================================
local function createHeadLabels(char)
    ...
    local old1 = char:FindFirstChild("PretoSpeedLabel")
    if old1 then old1:Destroy() end
    -- Remove any old Discord label
    local oldDiscord = char:FindFirstChild("PretoDiscordLabel")
    if oldDiscord then oldDiscord:Destroy() end

    local speedGui = Instance.new("BillboardGui")
    speedGui.Name = "PretoSpeedLabel"
...
-- ============================================================
-- ADD SPEED LABELS FOR OTHER PLAYERS
-- ============================================================
...
local function createPlayerSpeedLabel(plr)
    ...
    local speedGui = Instance.new("BillboardGui")
    speedGui.Name = "PretoSpeedLabel_Other"
    speedGui.Adornee = head
    speedGui.Size = UDim2.new(0, 160, 0, 28)
    speedGui.StudsOffset = Vector3.new(0, 2.8, 0)
    speedGui.AlwaysOnTop = true
...
RunService.RenderStepped:Connect(function()
    ...
    if not tpLockEnabled and not autoLeftEnabled and not autoRightEnabled and not batV2Enabled then
...
-- MOBILE BUTTONS
-- ============================================================
local MOB_POS_FILE = "pretobtnpos.json"
...
local function buildMobileButtons()
    ...
    local buttonOrder = {
        ...
        -- removed Taunt button
    }
    local mobGui = Instance.new("ScreenGui")
    mobGui.Name = "PretoMobileButtons"; mobGui.ResetOnSpawn = false
    ...
    for i, def in ipairs(buttonOrder) do
        ...
        btn.MouseButton1Click:Connect(function()
            ...
            if def.key == "carrySpeed" then
                toggleCarryMode(); setOn(speedMode)
                if mobBtnRefs.lagger then mobBtnRefs.lagger(laggerToggled) end
                if mobBtnRefs.laggerCarry then mobBtnRefs.laggerCarry(laggerCarryToggled) end
                saveConfig(); return
            end
...
local function createStealBar()
    for _, n in ipairs({"PretoStealBar"}) do
    ...
    local function loadStealBarPos()
        ...
        local ok, cfg = pcall(function() return HS:JSONDecode(readfile("PretoDuelsConfig.json")) end)
...
local function loadConfig()
    if not (isfile and isfile("PretoDuelsConfig.json")) then return end
    local ok, cfg = pcall(function() return HS:JSONDecode(readfile("PretoDuelsConfig.json")) end)
    ...
    if cfg.autoSwitchSpeed ~= nil then autoSwitchSpeedEnabled = cfg.autoSwitchSpeed end
...
-- ============================================================
if _G.PRETO_LOADED then return end
_G.PRETO_LOADED = true

-- ============================================================
-- BUILD PRETO DUELS GUI
-- ============================================================
local function buildGui()
    ...
    for _, name in ipairs({"PretoHub", "PretoMobileButtons", "PretoStealBar", "PretoRagdollTimer"}) do
    ...
    gui.Name = "PretoHub"; gui.ResetOnSpawn = false
    ...
    bgImage.Image = "rbxassetid://72639678834758"
    ...
    Title.BackgroundTransparency = 1; Title.Text = "PRETO DUELS"
    ...
    local TabList = {"Speed","Combat","Mechns","Visual","Extra","Songs"}
    ...
    mkBoxRow(sfSpeed, "Normal Speed", NS, function(v) if v>0 and v<=500 then NS=v end; saveConfig() end)
    mkBoxRow(sfSpeed, "Carry Speed", CS, function(v) if v>0 and v<=500 then CS=v end; saveConfig() end)
    ...
    mkToggle(sfSpeed, "Auto Switch Speed", function(on) autoSwitchSpeedEnabled = on; saveConfig() end, autoSwitchSpeedEnabled)
    ...
    -- ============================================================
    -- SAVE CONFIG BUTTON (added as per request)
    -- ============================================================
    mkSect(sfExtra, "Configuration")
    do
        local row = mkRow(sfExtra)
        mkLabel(row, "Save Config")
        local saveBtn = Instance.new("TextButton", row)
        saveBtn.Size = UDim2.new(0,80,0,24); saveBtn.Position = UDim2.new(1,-85,0.5,-12)
        saveBtn.BackgroundColor3 = BTN_ACT; saveBtn.BorderSizePixel = 0
        saveBtn.Text = "Save"; saveBtn.TextColor3 = WHITE
        saveBtn.Font = Enum.Font.GothamBold; saveBtn.TextSize = 10
        saveBtn.AutoButtonColor = false; saveBtn.ZIndex = 5
        Instance.new("UICorner", saveBtn).CornerRadius = UDim.new(0,5)
        local stroke = Instance.new("UIStroke", saveBtn); stroke.Color = WHITE; stroke.Thickness = 1
        saveBtn.MouseButton1Click:Connect(function()
            saveConfig()
            TS:Create(saveBtn, TweenInfo.new(0.1), {BackgroundColor3 = WHITE}):Play()
            TS:Create(saveBtn, TweenInfo.new(0.1), {TextColor3 = BLACK}):Play()
            task.delay(0.15, function()
                TS:Create(saveBtn, TweenInfo.new(0.1), {BackgroundColor3 = BTN_ACT}):Play()
                TS:Create(saveBtn, TweenInfo.new(0.1), {TextColor3 = WHITE}):Play()
            end)
        end)
    end

    -- SONGS TAB
    mkSect(sfSongs, "Songs")
    local songIds = {
        "108172611858503",
        "110646136220877",
        "114053505636209",
    ...
    local function createSongHandler(songId)
        return function()
    ...
    for i, id in ipairs(songIds) do
        ...
        local bs = Instance.new("UIStroke", playBtn)
        bs.Color = WHITE
        bs.Thickness = 1
        playBtn.MouseButton1Click:Connect(createSongHandler(id))
    end
    ...
    -- No intro - show GUI instantly
    mainFrame.Visible = true
    miniToggleBtn.Visible = false
    print("PRETO DUELS GUI Fully Loaded")
end
...
print("PRETO DUELS Fully Loaded – Bat V2 uses your specified settings: Chase 58, Hit 8, Cooldown 0.35, Height 3.7, Y -70..110")
